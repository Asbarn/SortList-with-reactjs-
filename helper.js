...
function tableSortByPrice(mass, choose) {
    if (choose == 1) {
        return mass.filter(function (value) {
            return value.props.product.price >= 150 && value.props.product.price < 400;
        });
    } else if (choose == 2) {
        return mass.filter(function (value) {
            return value.props.product.price >= 400 && value.props.product.price < 500;
        });
    } else if (choose == 3) {
        return mass.filter(function (value) {
            return value.props.product.price >= 500 && value.props.product.price < 600;
        });
    }
    return mass;
}
